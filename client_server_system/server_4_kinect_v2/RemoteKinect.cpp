...
RemoteKinect::RemoteKinect(char *ipAddr, int port) : _status(0), _hsock(0), _threadRunning(false)
{
	...
	calib_conn[0] = 2;
	calib_conn[1] = 0;
	//calib_conn[2] = 1; // for point cloud rendering mode
	calib_conn[2] = 1500; // for calibration
...
DWORD WINAPI RemoteKinect::getRemoteData(void *basept)
{
	...
	if (eye_pos[2] == 1)
	{
		...
		dataLeft = bp->_pointcloudX_Frame.size().height * bp->_pointcloudX_Frame.size().width * sizeof(unsigned short);
	...
	{
		// (1)a This is for tracking and receiving path use, this step is executed only when calib_conn[1] = 1 (8) Receive 1 unsigned long on the size of the depth bitstream
		...
		printf("datasize: %d\n", pathSize);
		dataLeft = bp->_pathMat.size().height * bp->_pathMat.size().width * sizeof(unsigned short);
...
void RemoteKinect::setPathTrackingSignal(int x)
{
	...
	{
		printf("before: %d, %d, %d\n", calib_conn[0], calib_conn[1], calib_conn[2]);
		calib_conn[1] = x;
		calib_conn[2] = 1;
